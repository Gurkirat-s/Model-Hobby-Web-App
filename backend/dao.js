...
const GET_ALL_PRODUCTS =
  "SELECT P.*, C.name as category, V.name as vendor " +
...
module.exports = {
  ...
  getVendors(query, success, failure = console.log) {
    let statement = [GET_ALL_VENDORS];
    db.all(GET_ALL_VENDORS, (err, rows) => {
      if (err == null) {
        success(rows);
      } else {
        failure(err);
  ...
  getCatalog(query, success, failure = console.log) {
    /*------*/
    let statement = [GET_ALL_CATEGORIES];

    if (query.id) {
      const category = " WHERE id = ?";
      statement.push(category);
      const values = [];
      ...
      db.all(GET_ALL_CATEGORIES, (err, rows) => {
        //console.log(statement);
        if (err == null) {
          success(rows);
        } else {
